...
class Twofish {
	...
	_createHelper() {
		...
		return {
			encrypt: (message, key) => {
				return toBase64(algo.encrypt(stringToByteArray(key), stringToByteArray(message)));
			},
			decrypt: (ciphertext, key) => {
				return byteArrayToString(algo.decrypt(stringToByteArray(key), fromBase64(ciphertext)));
			}
		}
	}
}
